...
# establish local storage location
if not os.path.exists(conf["local_image_dir"]):
	os.makedirs(conf["local_image_dir"])
...
for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	if text == "Occupied":
		...
		if(timestamp - lastUploaded).seconds >= conf["min_upload_seconds"]:
			...
			motionCounter += 1
			print("[MINIMUM UPLOAD INCREMENT REACHED]")
			
			# check to see if number of frames with motion is
			# high enough
			if motionCounter >= conf["min_motion_frames"]:
				print("[FRAME THRESHOLD REACHED]")
				# check to see if dropbox should be used
				# write temporary image file
				t = TempImage()
				cv2.imwrite(t.path, frame)		
	
				cv2.imwrite(conf["local_image_dir"] + ts + ".jpg", frame)
				t.cleanup()	
